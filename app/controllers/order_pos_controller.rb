...
)orders_temp on users.id = orders_temp.user_id LEFT JOIN inventory_orders on orders_temp.id = inventory_orders.order_id LEFT JOIN inventories on inventory_orders.inventory_id = inventories.id  WHERE orders_temp.readyorpo = 'PO' AND orders_temp.status = ? GROUP BY orders_temp.id,orders_temp.url_id, orders_temp.payment, orders_temp.status,orders_temp.ongkir,orders_temp.discount, users.name ORDER BY id", params[:order][:status]])  
  else 
...
)orders_temp on users.id = orders_temp.user_id LEFT JOIN inventory_orders on orders_temp.id = inventory_orders.order_id LEFT JOIN inventories on inventory_orders.inventory_id = inventories.id  WHERE orders_temp.readyorpo = 'PO' GROUP BY orders_temp.id, orders_temp.url_id,orders_temp.payment, orders_temp.status, orders_temp.discount, orders_temp.ongkir, users.name ORDER BY id")
   end
