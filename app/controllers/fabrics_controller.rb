class FabricsController < ApplicationController
	...
	if(params[:type] && params[:warna])
     ...
     @image = ProductImage.joins(:inventory).select('product_images.prod_img,product_images.id,product_images.inventory_id,inventories.name,inventories.category,inventories.fabrictype,inventories.quantity,inventories.satuan,inventories.sellprice').where(:inventories=>{:fabrictype => params[:type]}).where(:displaypic => 1).where("inventories.prodtype = 'Fabric'").where("inventories.quantity > ?", 0).paginate(:page => params[:page],:per_page => 24)
        ...
        @image = ProductImage.joins(:inventory).select('product_images.prod_img,product_images.id,product_images.inventory_id,inventories.name,inventories.category,inventories.fabrictype,inventories.quantity,inventories.satuan,inventories.sellprice').where(:inventories=>{:category => params[:fabrics][:category]}).where(:displaypic => 1).where("inventories.prodtype = 'Fabric'").where("inventories.quantity > ?", 0).paginate(:page => params[:page],:per_page => 24)
